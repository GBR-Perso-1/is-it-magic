...
| repo-git-trigger-workflow | STACK-LEAKED | `app/api/mcp/infra`→`*-deploy.yml`; example org `Rise-4` — see Cluster 2 |
| repo-git-env | STACK-LEAKED | `AZURE_WEBAPP_NAME`, `.github/appsettings-value/`, `.tf` — see Clusters 2 & 3 |
...
| repo-security-scan | WHOLESALE | Title "Scan a **Rise** repository"; hard-locked to `Rise-4` org |
...
- 2026-06-03 — `session-to-skill` rewritten into base (GENERIC, surgical leak-scrub): fixed `_ux-rules.md`
  reference to `${CLAUDE_PLUGIN_ROOT}/...`; removed dead machine-specific path check in Phase 5; kept
  `<Scope>.Applications/` inline discovery (consistent with `project-locator`).
