...
Identify build/runtime config files present and read them in full to understand what is exposed to the frontend bundle or production runtime. Selection is stack-aware: always check the **baseline**; then for each stack named in `STACK_HINT`, add that stack's set. If `STACK_HINT` is empty or unrecognised, add **all** stack sets (no coverage is lost when the stack is unknown).

- **Baseline (always):** `**/.env.production`, `**/*.env.production`, `**/cors*`
- **node / vue:** `vite.config.ts`, `vite.config.js`, `webpack.config.js`, `next.config.js`, `angular.json`
- **dotnet:** `**/appsettings*.Production.json`, `**/Program.cs`, `**/Startup.cs`

**Surface mode file targets**: the config files selected above.
