...
**Surface-mode file targets** are stack-selected. Always scan the **baseline** set; then for each stack named in `STACK_HINT`, add that stack's set. If `STACK_HINT` is empty or unrecognised, add **all** stack sets (no coverage is lost when the stack is unknown).

- **Baseline (always):** `**/.env`, `**/.env.*`, `**/secrets.*`, `**/*secret*`, `**/credentials*`, `**/*.pem`, `**/*.key`, `**/*.pfx`
- **dotnet:** `**/appsettings*.json`, `**/Controllers/**`, `**/Handlers/**`
- **node / vue:** `**/.npmrc`
- **python:** `**/settings.py`, `**/*.cfg`, `**/.pypirc`
- **terraform:** `**/*.tfvars`, `**/terraform.tfvars`, `**/*.tfstate`
...
If `TRACKED` and the `.gitignore` contains a pattern that *should* match this file but doesn't (e.g. covers `credentials.ps1` but not `credentials-prod.ps1`), note the gap in `description`: `The .gitignore pattern does not cover this filename variant — file is actively tracked.` Keep the original severity.
